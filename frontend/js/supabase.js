...
async function sbGetDestinations(category = 'all', search = '') {
    ...
    if (search) {
        ...
        list = list.filter((d) => [d.name, d.location, d.region, d.shortDesc, d.fullDesc].filter(Boolean).some((f) => String(f).toLowerCase().includes(s)) ||
...
async function __sbFetch(endpoint, options) {
    ...
    if (method === 'GET') {
        ...
        if (dMatch)
            return sbGetDestination(decodeURIComponent(dMatch[1]));
        if (ep === '/festivals')
            return sbGetFestivals();
        const fMatch = ep.match(/^\/festivals\/([^/?]+)/);
        if (fMatch)
            return sbGetFestival(decodeURIComponent(fMatch[1]));
        const iMatch = ep.match(/^\/info\/([^/?]+)/);
        if (iMatch)
            return sbGetInfo(decodeURIComponent(iMatch[1]));
        if (ep === '/regions')
            return sbGetRegions();
        const rMatch = ep.match(/^\/regions\/([^/?]+)/);
        if (rMatch)
            return sbGetRegion(decodeURIComponent(rMatch[1]));
        if (ep === '/wishlist')
            return sbGetWishlist();
        const bMatch = ep.match(/^\/bookings\/([^/?]+)/);
        if (bMatch)
            return sbGetBooking(decodeURIComponent(bMatch[1]));
        if (ep === '/health')
            return { success: true, data: { status: 'healthy', service: 'Ghana Kora Vista Tours API' } };
    }
    if (method === 'POST') {
        if (ep === '/bookings')
            return sbCreateBooking(body || {});
        if (ep === '/wishlist')
            return sbToggleWishlist((body || {}).destinationId);
        if (ep === '/itinerary')
            return sbGenerateItinerary(Number((body || {}).days) || 7, (body || {}).style || 'balanced');
    }
