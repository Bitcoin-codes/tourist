...
async function renderDestinations() {
    ...
    try {
        ...
        // Never show a guest our stack. This endpoint fails transiently (a
        // redeploy mid-request, or a slow cold start), so say so plainly and
        // offer a retry rather than leaving them at a dead end.
        grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:48px 16px;background:var(--bg-card);border-radius:var(--radius-lg);border:1px solid var(--border-light);">
...
function buildItinerary(days, style, dests) {
...
function chatResponse(q) {
    ...
    if (l.includes('pay') || l.includes('card'))
        return `<p><strong>Payment:</strong> Reserve with nothing upfront and settle on arrival (Visa, MoMo or cash), or pay a deposit online by card or Mobile Money.</p>`;
...
async function renderAttractionPins(svg, regions) {
    return; // bell: attraction pins removed from the map
    try {
        const res = await getDestinations('all', '');
        ...
        const BOUNDS = { x0: 23.6, x1: 776.4, y0: 28.8, y1: 911.2 };
