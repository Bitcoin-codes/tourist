...
export function initFestivals(): void {
  renderFestivals();
}
...
async function renderFestivals(): Promise<void> {
  ...
  try {
    ...
    container.innerHTML = festivals.map((item: Festival) => `
...
export async function openFestivalModal(id: string): Promise<void> {
  ...
  try {
    const response = await getFestivals();
    const festival = response.data.find((f: Festival) => f.id === id);
    if (!festival) return;
